...
function App() {
  ...
  const hadndleClickApiCall = async (userInput) => {
    try {
      setLoading(true);
  ...
  function submitHandler(userInput) {
    hadndleClickApiCall(userInput);
  }

  return (
    <>
      <DiaryInput isLoading={isLoading} onSubmit={submitHandler} />
      <div>data: {JSON.stringify(data)}</div>
      <div>isLoaidng: {isLoading ? "loading" : "fin"}</div>
      <button onClick={hadndleClickApiCall}>gpt call</button>
    </>
  );
}
